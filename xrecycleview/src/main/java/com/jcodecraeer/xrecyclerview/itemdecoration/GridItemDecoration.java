...
public class GridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private Drawable mDividerH, mDividerV;

    public GridItemDecoration(Context context) {
        final TypedArray a = context.obtainStyledAttributes(ATTRS);
        mDividerH = mDividerV = a.getDrawable(0);
        a.recycle();
    }

    public GridItemDecoration(Context context, int drawableIdH, int drawableIdV) {
        mDividerH = context.getResources().getDrawable(drawableIdH);
        mDividerV = context.getResources().getDrawable(drawableIdV);
    }
    ...
    public void getItemOffsets(Rect outRect, int itemPosition, RecyclerView parent) {
        ...
        } else if (isLastColum(parent, itemPosition, spanCount, childCount))// 如果是最后一列，则不需要绘制右边
        {
            ...
            outRect.set(0, 0, mDividerV.getIntrinsicWidth(),
                    mDividerH.getIntrinsicHeight());
        }
    }
}